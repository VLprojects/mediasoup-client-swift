...
then
	...
	echo 'on Linux systems try this:'
	echo '  * run "sudo apt-get install python-is-python3"'
	exit
fi
...
export BUILD_DIR=$(pwd)/build_android
echo "BUILD_DIR = $BUILD_DIR"
export OUTPUT_DIR=$(pwd)/bin_android
...
platform_args='target_environment="device" target_cpu="x64"'
gn gen $BUILD_DIR/WebRTC/x86_64 --args="${platform_args}${gn_args}"

platform_args='target_environment="simulator" target_cpu="arm"'
gn gen $BUILD_DIR/WebRTC/armeabi-v7a --args="${platform_args}${gn_args}"

platform_args='target_environment="simulator" target_cpu="arm64"'
gn gen $BUILD_DIR/WebRTC/arm64-v8a --args="${platform_args}${gn_args}"
